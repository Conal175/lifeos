...
export function Reports() {
  const { data: transactions = [], isLoading: loadTx } = useTableData<Transaction>('transactions', 'date', 365);
  const { data: tasks = [], isLoading: loadTa } = useTableData<Task>('tasks');
  const { data: budgets = [], isLoading: loadB } = useTableData<Budget>('budgets');
  const { data: savingsGoals = [], isLoading: loadS } = useTableData<SavingsGoal>('savings_goals');
  const isLoading = loadTx || loadTa || loadB || loadS;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border dark:border-gray-700">
          <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><TrendingUp className="w-5 h-5 text-green-600" /></div><span className="text-sm text-gray-500">Tổng thu</span></div>
          <p className="text-2xl font-bold text-green-600">{totalIncome.toLocaleString('vi-VN')} ₫</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border dark:border-gray-700">
          <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 bg-red-100 rounded-lg flex items-center justify-center"><TrendingDown className="w-5 h-5 text-red-600" /></div><span className="text-sm text-gray-500">Tổng chi</span></div>
          <p className="text-2xl font-bold text-red-600">{totalExpense.toLocaleString('vi-VN')} ₫</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border dark:border-gray-700">
          <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center"><Percent className="w-5 h-5 text-purple-600" /></div><span className="text-sm text-gray-500">Tiết kiệm</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border dark:border-gray-700">
          <h3 className="font-semibold mb-4 dark:text-white">💰 Luồng tiền</h3>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border dark:border-gray-700">
          <h3 className="font-semibold mb-4 dark:text-white">📂 Phân bổ chi tiêu</h3>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
