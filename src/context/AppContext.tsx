...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // Chỉ cần dùng onAuthStateChange, nó sẽ tự động bắt INITIAL_SESSION khi tải trang
    const { data: { subscription } } = onAuthStateChange(async (event, session) => {
      console.log(`🔐 Sự kiện Supabase: ${event}`);

      if (session?.user) {
        // CHỐNG SPAM: Nếu đã/đang tải dữ liệu cho user này rồi, NGĂN KHÔNG CHO GỌI LẠI
        ...
        setIsLoading(true);
        fetchedUserId.current = session.user.id; // Đánh dấu là user này đang được xử lý
        await loadUserDataFromSupabase(session.user.id, session.user);

      } else if (event === 'SIGNED_OUT') {
        setUser(null);
        fetchedUserId.current = null; // Xóa trí nhớ chống spam khi đăng xuất
